...
- Beyond the curated entries, the build **mirrors thousands of MCP servers from upstream open directories** (the official [MCP Registry](https://registry.modelcontextprotocol.io) and [Glama](https://glama.ai)) at deploy time. Registry servers come in fully installable; Glama servers are added as labelled `reference` pointers to their repos. Aggregated data is published into the catalog but **never committed to git** — curated entries stay the source of truth. See [`ARCHITECTURE.md`](ARCHITECTURE.md#phase-15--build-time-aggregation-from-upstream-directories-implemented) → Phase 1.5. Each entry carries a `provenance` field (`curated` / `mcp-registry` / `glama`) so clients can badge or filter by source.
